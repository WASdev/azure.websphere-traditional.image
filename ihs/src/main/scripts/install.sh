...
# Create master password file required for headless secure storage on IM 1.9+
...
${IM_INSTALL_DIRECTORY}/eclipse/tools/imutilsc saveCredential -secureStorageFile storage_file \
    ...
    -userName "$userName" -userPassword "$password" -url "$REPOSITORY_URL"

# Check whether IBMid is entitled or not
if [ $? -eq 0 ]; then
    output=$(${IM_INSTALL_DIRECTORY}/eclipse/tools/imcl listAvailablePackages \
        -repositories "$REPOSITORY_URL" -secureStorageFile storage_file \
        -masterPasswordFile master_password_file)
    if echo "$output" | grep "$WAS_ND_VERSION_ENTITLED"; then
        ...
        rm -rf storage_file && rm -rf log_file && rm -rf master_password_file
        exit 1
    else
...
${IM_INSTALL_DIRECTORY}/eclipse/tools/imcl updateAll -repositories "$REPOSITORY_URL" \
    -acceptLicense -log log_file -installFixes recommended -secureStorageFile storage_file \
    -masterPasswordFile master_password_file -preferences $SSL_PREF,$DOWNLOAD_PREF -showProgress
